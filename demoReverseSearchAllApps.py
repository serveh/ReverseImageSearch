...
def check_image_type(imgfile):
    imagetypes = ['.png', '.jpg', '.jpeg', '.bmp']
    target = '.png'
    bResult = True
    filename, extension = splitext(imgfile)
    extension = extension.lower()
    try:
        if extension not in imagetypes:
            im = Image.open(filename + extension)
            filereturn = filename + target
            im.save(filereturn)
        else:
            filereturn = filename + extension
    except OSError:
        bResult = False
        print('Cannot convert %s' % file)
    return filereturn, bResult
            ...
            for imgfn in os.listdir(imageDir):
                ...
                if imgfn not in procImageList:
                    try:
                        ...
                        fileName, bChange = check_image_type(fileName)
                        if bChange:
                            imSz = img.size
